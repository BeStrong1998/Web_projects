...
def create_app():
    ...
    def admin_index():
        if current_user.is_admin: #Если в функции is_admin, role == 'admin'
            return render_template('admin.html', admin='Панель управления для администратора!')  #Показываем скрытую страницу пользователю если поле role = 'admin'
        else:
            return 'Вы не администратор!'  #Не показываем скрытую страницу пользователю так как поле role = другому значению
